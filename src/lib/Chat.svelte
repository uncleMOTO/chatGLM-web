...
<nav class="level chat-header">
  ...
  <div class="level-right">
    <p class="level-item">
      <button class="button is-warning" on:click={() => { clearMessages(chatId) }}><span class="greyscale mr-2">🗑️</span> 清除信息</button>
    </p>
    <p class="level-item">
      <button class="button is-success" on:click={() => { window.open("https://www.126.com") }}><span class="greyscale mr-1">📪</span> 126邮箱</button>
      <button class="button is-danger"  on:click={() => { window.open("https://mail.qq.com/") }}><span class="greyscale mr-1">🐧</span> qq邮箱</button>
      <button class="button is-info" on:click={() => { window.open("https://www.outlook.com/") }}><span class="greyscale mr-1">📮</span> outlook</button>
    </p>
  </div>
</nav>
...
<form class="modal" bind:this={chatNameSettings} on:submit={saveChatNameSettings}>
  ...
  <div class="modal-card">
    <header class="modal-card-head">
      <p class="modal-card-title">为该对话输入新名字</p>
    </header>
    <section class="modal-card-body">
      <div class="field is-horizontal">
        <div class="field-label is-normal">
          <label class="label" for="settings-chat-name">新名字:</label>
        </div>
        <div class="field-body">
          <div class="field">
            <input
    ...
    <footer class="modal-card-foot">
      <input type="submit" class="button is-info" value="保存" />
